...
export default function Admin() {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id)
    try {
      await sbFetch(`rsvps?id=eq.${id}`, { method: 'DELETE' })
  ...
  const exportCSV = () => {
    const rows = [['Name','Attending','Date'].join(','), ...rsvps.map(r =>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background: '#0a0a0a' }}>
        ...
        >
          ...
          <div className="rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,168,76,0.2)' }}>
            <input
              type="password"
          ...
          <a href="/" className="block mt-6 font-mono text-[0.65rem] tracking-widest text-cream/25 hover:text-cream/40 transition-colors">
            ← Back to invitation
          </a>
        </motion.div>
      </div>
  ...
  return (
    <div className="min-h-screen px-6 py-12" style={{ background: '#0a0a0a', fontFamily: 'Cormorant Garamond, serif' }}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="rounded-xl p-4 mb-6" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <motion.div
              ...
              style={{ background: count >= MAX_ATTENDANCE ? '#ef4444' : 'linear-gradient(90deg, #c9a84c, #e879f9)' }}
            />
          </div>
        </div>
        ...
        {/* Table */}
        {loading ? (
          <div className="text-center py-16 font-cormorant italic text-cream/30 text-xl">Loading RSVPs...</div>
        ) : filtered.length === 0 ? (
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="grid grid-cols-12 px-4 py-2 font-mono text-[0.6rem] tracking-widest text-cream/30 uppercase"
              ...
              {filtered.map((r, i) => (
                ...
                >
                  ...
                  <span className="col-span-4 font-mono text-[0.65rem] text-cream/40">
                    {new Date(r.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: '2-digit', hour: '2-digit', minute: '2-digit' })}
                  </span>
                  <div className="col-span-2 flex justify-end">
                    <button
                      onClick={() => handleDelete(r.id)}
                      disabled={deleting === r.id}
                      className="font-mono text-[0.6rem] tracking-wider px-3 py-1 rounded-lg transition-all hover:bg-red-900/30"
